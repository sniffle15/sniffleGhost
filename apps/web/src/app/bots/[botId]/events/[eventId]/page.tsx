...
export default function EventDetailPage() {
  ...
  async function load() {
    const data = await authFetch(`/events/${eventId}`);
    const versionData = await authFetch(`/events/${eventId}/versions`);
    setEventConfig(data);
  ...
  async function createVersion() {
    await authFetch(`/events/${eventId}/versions`, { method: "POST", body: JSON.stringify({}) });
    await load();
  }
  ...
  async function deleteEvent() {
    ...
    try {
      ...
      router.push(`/bots/${botId}/events`);
    } catch (error: any) {
      setDeleteStatus(error?.message ?? "Failed to delete event.");
      setIsDeleting(false);
    }
  }
    ...
    <div className="space-y-4">
      <section className="dash-panel p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="dash-subtitle">Event</p>
            <h2 className="dash-title">{eventConfig?.name ?? "event"}</h2>
            <p className="mt-1 text-sm dash-muted">{eventConfig?.description}</p>
          </div>
          <div className="flex items-center gap-2">
            <Badge className="bg-white/10 text-fog/70 border-white/20">
              {formatEventTypeLabel(eventConfig?.eventType) ?? "EVENT"}
            </Badge>
            <Button
              variant="ghost"
              size="sm"
            ...
            <Button variant="outline" size="sm" asChild>
              <a href={`/bots/${botId}/events`}>Back</a>
            </Button>
          </div>
        </div>
      ...
      <section className="dash-panel p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h3 className="text-2xl font-display">Event Versions</h3>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={createVersion}>New Version</Button>
            {latest && (
              <Button variant="ghost" size="sm" asChild>
                <a href={`/bots/${botId}/events/${eventId}/builder?version=${latest.id}`}>Edit Latest</a>
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          {versions.map((version) => (
            <div key={version.id} className="dash-panel-soft flex items-center justify-between gap-3 p-3">
              <div>
                <p className="text-sm font-semibold text-fog">v{version.versionNumber}</p>
                <p className="text-xs dash-muted">{version.status}</p>
              </div>
              <Button variant="outline" size="sm" asChild>
                <a href={`/bots/${botId}/events/${eventId}/builder?version=${version.id}`}>Open Builder</a>
              </Button>
            </div>
          ))}
