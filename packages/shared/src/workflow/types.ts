...
export interface MessageCreateTriggerData extends BaseNodeData {
  eventType?:
    | "MESSAGE_CREATE"
    | "MESSAGE_DELETE"
    | "MESSAGE_UPDATE"
...
export type NodeData =
  | SlashCommandTriggerData
  | MessageCreateTriggerData
  | ReplyMessageData
  ...
  | LoopData
  | SetVariableData
  | GetPersistentVariableData
  | SetPersistentVariableData
  | DelayData
  | HttpRequestData
  | AddRoleData
  | RemoveRoleData
...
export interface ActionHandlers {
  reply: (content: string, options?: { ephemeral?: boolean; buttons?: MessageButton[]; selectMenus?: MessageSelectMenu[] }) => Promise<any>;
  sendChannel: (channelId: string, content: string, options?: { buttons?: MessageButton[]; selectMenus?: MessageSelectMenu[] }) => Promise<any>;
  sendDm: (content: string, options?: { targetUserId?: string; buttons?: MessageButton[]; selectMenus?: MessageSelectMenu[] }) => Promise<any>;
  sendEmbed: (embed: EmbedMessageData, options?: { buttons?: MessageButton[]; selectMenus?: MessageSelectMenu[] }) => Promise<any>;
  ...
  awaitInteraction?: (input: {
    ...
    | { kind: "select"; menuId: string; optionValue?: string }
    | null
  >;
  addRole: (roleId: string, options?: { reason?: string; targetUserId?: string }) => Promise<void>;
  removeRole: (roleId: string, options?: { reason?: string; targetUserId?: string }) => Promise<void>;
  log: (level: LoggerData["level"], message: string, meta?: Record<string, unknown>) => Promise<void>;
  httpRequest: (data: HttpRequestData) => Promise<{ status: number; body: unknown }>;
}
